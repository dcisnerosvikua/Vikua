...
class HrEmployeeOtraLine(models.Model): #2
    _name = "hr.employee.otra_line"

    employee_arc_id = fields.Many2one('hr.employee.arc')
    nro_mes = fields.Integer()
    mes = fields.Char()
    monto_asignacion = fields.Float(default=0)
    monto_base = fields.Float()
    porcentaje = fields.Float(default=0)
    monto_rete = fields.Float(default=0)
...
class HrEmployeeArc(models.Model): #2
    ...
    line_otra  = fields.One2many('hr.employee.otra_line','employee_arc_id', string='Lineas')
...
class WizardReport_5(models.TransientModel): # aqui declaro las variables del wizar que se usaran para el filtro del pdf
    ...
    tipo_retencion = fields.Selection([('islr_c', 'ISLR'),('faov_c', 'FAOV'),('sso_c','SSO'),('ince_c','INCES'),('rpe_c','RPE')])
    ...
    def print_rep_arc(self):
        ...
        y=self.env['hr.employee.otra_line'].search([])
        t.unlink()
        w.unlink()
        z.unlink()
        y.unlink()
        ...
        if lista:
            for det in lista:
                self.construye(det)
                if self.tipo_retencion=='islr_c':
                    payslip_line=self.env['hr.payslip.line'].search([('salary_rule_id.islr_c','=',True),('employee_id','=',det.employee_id.id),('date_from','>=',self.date_from),('date_to','<=',self.date_to)],order='date_from asc')
                if self.tipo_retencion=='faov_c':
                    payslip_line=self.env['hr.payslip.line'].search([('salary_rule_id.faov_c','=',True),('employee_id','=',det.employee_id.id),('date_from','>=',self.date_from),('date_to','<=',self.date_to)],order='date_from asc')
                if self.tipo_retencion=='sso_c':
                    payslip_line=self.env['hr.payslip.line'].search([('salary_rule_id.sso_c','=',True),('employee_id','=',det.employee_id.id),('date_from','>=',self.date_from),('date_to','<=',self.date_to)],order='date_from asc')
                if self.tipo_retencion=='ince_c':
                    payslip_line=self.env['hr.payslip.line'].search([('salary_rule_id.ince_c','=',True),('employee_id','=',det.employee_id.id),('date_from','>=',self.date_from),('date_to','<=',self.date_to)],order='date_from asc')
                if self.tipo_retencion=='rpe_c':
                    payslip_line=self.env['hr.payslip.line'].search([('salary_rule_id.rpe_c','=',True),('employee_id','=',det.employee_id.id),('date_from','>=',self.date_from),('date_to','<=',self.date_to)],order='date_from asc')
        ...
        if self.tipo_retencion=='islr_c':
            ...
            return {'type': 'ir.actions.report','report_name': 'hr_rep_arc_islr.reporte_otra','report_type':"qweb-pdf"}
        ...
        for j in range(13):
            if j!=0:
                ...
                if self.tipo_retencion=='islr_c':
                    ...
                    self.env['hr.employee.otra_line'].create(values)
    ...
    def actualiza(self,det,item):
        ...
        if self.tipo_retencion=='islr_c':
            ...
            busca=self.env['hr.employee.otra_line'].search([('nro_mes','=',nro_mes),('employee_arc_id','=',det.id)])
            if busca:
                for roc in busca:
                    valor=roc.monto_rete
                    valor=valor+item.amount

                    if self.tipo_retencion=='faov_c':
                        roc.monto_asignacion=valor/0.01
                        roc.monto_rete=valor
                        roc.porcentaje=0.01
                    if self.tipo_retencion=='sso_c' or self.tipo_retencion=='rpe_c':
                        roc.monto_asignacion=item.slip_id.sueldo
                        roc.monto_rete=valor
                    if self.tipo_retencion=='ince_c':
                        roc.monto_asignacion=valor/0.005
                        roc.monto_rete=valor
                        roc.porcentaje=0.005

...
class AccountMove(models.Model):
    ...
    def _compute_monto_conversion(self):
        ...
        for selff in self:
            ...
            if self.env.company.currency_id.id==selff.currency_id.id:
                lista_tasa = selff.env['res.currency.rate'].search([('currency_id', '=', self.env.company.currency_secundaria_id.id),('name','<=',selff.invoice_date)],order='id ASC')
                if lista_tasa:
    ...
    def _compute_monto_conversion_tax(self):
        ...
        for selff in self:
            ...
            if self.env.company.currency_id.id==selff.currency_id.id:
                lista_tasa = selff.env['res.currency.rate'].search([('currency_id', '=', self.env.company.currency_secundaria_id.id),('name','<=',invoice_selff.date)],order='id ASC')
                if lista_tasa:
                    for det in lista_tasa:
...
class  AccountMoveLine(models.Model):
    ...
    def _compute_monto_credit_conversion(self):
        ...
        self.env.company.currency_secundaria_id.id
        for selff in self:
            lista_tasa = selff.env['res.currency.rate'].search([('currency_id', '=', self.env.company.currency_secundaria_id.id),('name','<=',selff.move_id.invoice_date)],order='id ASC')
            if lista_tasa:
                for det in lista_tasa:
